...
function graduates(students) {
  // Code disini
  //!biasa, bikin result dulu sesuai format
  let result = {};

  //!loop students, jadiin students[i] ke var student biar gampang
  for (let i = 0; i < students.length; i++) {
    let student = students[i];
    //!kalau belum ada kelas di result, bikin dulu ya
    if (!result[student.class]) {
      result[student.class] = [];
    }
    //!udah ada kelas, cek kelulusan siswa, buat object baru, kalau lulus masukin var ke object, lalu...
    //!...push object ke aray kelas yg di dalam result
    if (student.score > 75) {
      let object = { name: "", score: 0 };
      object.name = student.name;
      object.score = student.score;
      result[student.class].push(object);
    }
  }
  return result;
}
