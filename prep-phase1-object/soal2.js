...
function economyChangeSummary(tradeActivity) {
  ...
  let newTradeActivity = [];
  for (let i = 0; i < tradeActivity.length; i++) {
    newTradeActivity.push(...tradeActivity[i]);
  }

  let trade = [];
  for (let i = 0; i < newTradeActivity.length; i++) {
    let tempArray = newTradeActivity[i].split("");
    if (tempArray.includes("+")) {
      trade.push([...newTradeActivity[i].split("+"), "+"]);
    } else {
      trade.push([...newTradeActivity[i].split("-"), "-"]);
    }
  }
  ...
  for (let i = 0; i < trade.length; i++) {
    if (trade[i][0] === "Jeff Bezos" && trade[i][2] === "+") {
      result.push({ name: "Jeff Bezos", deposit: duitJeff + (duitJeff * trade[i][1].slice(0, -1)) / 100, owner: "Amazon" });
      duitJeff += (duitJeff * trade[i][1].slice(0, -1)) / 100;
    } else if (trade[i][0] === "Larry Page" && trade[i][2] === "+") {
      result.push({ name: "Larry Page", deposit: duitLarry + (duitLarry * trade[i][1].slice(0, -1)) / 100, owner: "Google" });
      duitLarry += (duitLarry * trade[i][1].slice(0, -1)) / 100;
    } else if (trade[i][0] === "Jack Ma" && trade[i][2] === "+") {
      result.push({ name: "Jack Ma", deposit: duitJack + (duitJack * trade[i][1].slice(0, -1)) / 100, owner: "Alibaba" });
      duitJack += (duitJack * trade[i][1].slice(0, -1)) / 100;
    } else if (trade[i][0] === "Larry Page" && trade[i][2] === "-") {
      result.push({ name: "Larry Page", deposit: duitLarry - (duitLarry * trade[i][1].slice(0, -1)) / 100, owner: "Google" });
      duitLarry -= (duitLarry * trade[i][1].slice(0, -1)) / 100;
    } else if (trade[i][0] === "Jack Ma" && trade[i][2] === "-") {
