...
function countProfit(shoppers) {
  ...
  for (let obj = 0; obj < shoppers.length; obj++) {
    for (let index = 0; index < result.length; index++) {
      //!cek jika ada product dibeli
      if (shoppers[obj].product === result[index].product) {
        //!cek jika jumlah yg dibeli masih ada stocknya
        if (shoppers[obj].amount <= result[index].leftOver) {
          //!jika true semua, leftover berkurang, masukkan nama shopper, tambahkan total profit
          result[index].leftOver -= shoppers[obj].amount;
          result[index].shoppers.push(shoppers[obj].name);
          result[index].totalProfit += shoppers[obj].amount * listBarang[index][1];
        }
      }
    }
  }
